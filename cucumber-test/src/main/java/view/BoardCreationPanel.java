...
//		repaint();
	...
	/**Sets the size (length of each side) of each cell based on the number of rows and columns of 
	 * the board */
	...
	/**Sets the width and height of the board creation panel from 
	 * the given columns, rows and the number of pixels per cell, which are fields of the panel  */
	private void setWidthHeight() {
		width = cols*pixelsPerCell;
		height = rows*pixelsPerCell;
	}
	
	/**This method goes through all positions of a column and a row and creates a cell of type JButton.  
	 * Each cell is then placed on the grid with the corresponding values of row and column, 
	 * in the panel.   */
...
//The icon with the image of an empty cell is assigned to a cell
					cell.setIcon(icon);
//Set the size of the cell equal to the icon (which are the dimensions (pixelsPerCell, pixelsPerCell)
...
//This is done to know the position of each button, for effects of removing certain buttons 
//For Environment Elements that cannot appear twice, i.e, Checkpoints and Teleporters
					cellButtons[y][x]=cell;
					
//Don't add an actionListener to buttons in the last row
					if(row!=11) {
					
						cell.addActionListener(e -> {
							
//get a reference to the button that is being clicked
...
//Check if an element was selected in the ElementSelectionPanel
...
//Set save button enabled, so that the user can save the board, since something was added
...
//Instantiate the EE that is selected
	...
	//If there is an Environment Element in the position pos, of the cell clicked 
		if(newBoard.hasEElementAt(pos)) {
			...
			if(newElem instanceof Checkpoint) {
				...
				//remove checkpoint with the same number in newBoard
				...
				//remove teleporter with the same isSending attribute in newBoard
